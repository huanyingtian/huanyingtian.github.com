...
/* 判断是否存在文件 */
$tplfile = INDEX_TEMPLATE."case_".$mod.".".$tplext;
$widgetfile = "./source/widget/case_".$mod.".php";
...
if(isset($cid) && $cid != '' && is_numeric($cid)){
	$arr = Mod_Url::firstcid($cid,'casecate');
	$category_son = Mod_Case::CategorySon($arr['cid']);
	$tpl->assign("first_cname",$arr['cname']);
  }else{
	$category_son = Mod_Case::category();
	$tpl->assign("first_cname",$LANVAR['case']);
}	
if($cid > 0){
    $tpl->assign('cate_banner',$commons->banners($cid,'casecate'));
} 
