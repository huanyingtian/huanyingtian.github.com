...
function volist(){
	Core_Auth::checkauth("casecatevolist");
	global $db,$tpl,$page;
    // $pagesize	= 30;
	$searchsql	= " WHERE 1=1";
	// $countsql	= "SELECT COUNT(cid) FROM ".DB_PREFIX."casecate".$searchsql;
 //    $total		= $db->fetch_count($countsql);
 //    $pagecount	= ceil($total/$pagesize);
	// $nextpage	= $page+1;
	// $prepage	= $page-1;
	// $start		= ($page-1)*$pagesize;
	// $sql		= "SELECT * FROM ".DB_PREFIX."casecate".
	...
	{
		$cate[$key]['casecount'] = $db->fetch_count("SELECT COUNT(id) FROM ".DB_PREFIX."case WHERE cid=".$value['cid']."");
	...
	$tpl->assign("total",count($cate));
}

function add(){
	Core_Auth::checkauth("casecateadd");
	global $tpl,$db;
	$orders  = $db->fetch_newid("SELECT MAX(orders) FROM ".DB_PREFIX."casecate",1);
	$cid = isset($_GET['cid']) && !empty($_GET['cid']) && is_numeric($_GET['cid']) ? intval($_GET['cid']) : '';
	$tpl->assign("cate_select", Core_Mod::tree_select("casecate", $cid, "rootid"));
...
function edit(){
	...
	$sql	= "SELECT * FROM ".DB_PREFIX."casecate WHERE cid=$id";
	$cate	= $db->fetch_first($sql);
	if(!$cate){
		msg::msge('数据不存在!');
	}else{
		//$tpl->assign("cate_select", Core_Mod::tree_select("casecate", $id, "id"));
...
function saveadd(){
	...
	{
		$root_sql = "SELECT depth FROM ".DB_PREFIX."casecate WHERE cid=$rootid";
	...
	$cid	= $db->fetch_newid("SELECT MAX(cid) FROM ".DB_PREFIX."casecate",1);
	...
	$result = $db->insert(DB_PREFIX."casecate",$array);
	if($result){
		Core_Command::runlog("","添加资讯分类成功[$cname]",1);
		msg::msge('保存成功','xycms_casecate.php');
	}else{
		msg::msge('保存失败');
	}
}
...
function saveedit(){
	...
	$result = $db->update(DB_PREFIX."casecate",$array,"cid=$id");
	if($result){
		Core_Command::runlog("","编辑资讯分类成功[id=$id]");
		msg::msge('编辑成功','xycms_casecate.php?'.$GLOBALS['comeurl']);
	}else{
		msg::msge('编辑失败!');
	}
}
...
function delcates(){
	...
	for($ii=0;$ii<count($arrid);$ii++){
		...
		if(Core_Fun::isnumber($id)){
			if(Core_Mod::exist_child($id,"casecate")){
				msg::msge('对不起，所删分类下含有子类，不能删除！');
			}else{
				$db->query("DELETE FROM ".DB_PREFIX."casecate WHERE cid=$id");
			}
		}
	}
	Core_Command::runlog("","删除案例分类成功[cid=$arrid]");
	msg::msge('删除成功','xycms_casecate.php');
}

function del(){
	Core_Auth::checkauth("casecatedel");
	...
	for($ii=0;$ii<count($arrid);$ii++)
	{
		...
		if(Core_Fun::isnumber($id))
		{
			$sql	= "SELECT img FROM ".DB_PREFIX."casecate WHERE cid=$id";
	...
	*/
	...
	else
	{
		...
		{
			$GLOBALS['db']->query("DELETE FROM ".DB_PREFIX."casecate WHERE cid=$id");
...
function updateajax($_id,$_action){
    ...
    if(Core_Fun::isnumber($_id)){
		...
		switch($_action){
			case 'flagopen':
				$db->query("UPDATE ".DB_PREFIX."casecate SET flag=1 WHERE cid=$_id");
				break;
			case 'flagclose':
				$db->query("UPDATE ".DB_PREFIX."casecate SET flag=0 WHERE cid=$_id");
				break;
			case 'eliteopen':
				$db->query("UPDATE ".DB_PREFIX."casecate SET elite=1 WHERE cid=$_id");
				break;
			case 'eliteclose':
				$db->query("UPDATE ".DB_PREFIX."casecate SET elite=0 WHERE cid=$_id");
				break;
			default:
				break;
		}
...
$tpl->display(ADMIN_TEMPLATE."casecate.tpl");
