...
function volist(){
    ...
    $pagesize	= 30;
	$searchsql	= " WHERE 1=1";
	$countsql	= "SELECT COUNT(cid) FROM ".DB_PREFIX."mapcate".$searchsql;
    $total		= $db->fetch_count($countsql);
    $pagecount	= ceil($total/$pagesize);
	$nextpage	= $page+1;
	$prepage	= $page-1;
	$start		= ($page-1)*$pagesize;
	$sql		= "SELECT * FROM ".DB_PREFIX."mapcate".
...
function add(){
	...
	global $tpl,$db;
	$orders  = $db->fetch_newid("SELECT MAX(orders) FROM ".DB_PREFIX."mapcate",1);
...
function edit(){
	...
	$sql	= "SELECT * FROM ".DB_PREFIX."mapcate WHERE cid=$id";
...
function saveadd(){
	...
	$result = $db->insert(DB_PREFIX."mapcate",$array);
	if($result){
		Core_Command::runlog("","添加单页分类成功[$cname]",1);
		msg::msge('保存成功','xycms_mapcate.php');
	}else{
...
function saveedit(){
	...
	$result = $db->update(DB_PREFIX."mapcate",$array,"cid=$id");
	if($result){
		Core_Command::runlog("","编辑单页分类成功[id=$id]");
		msg::msge('编辑成功',"xycms_mapcate.php?".$GLOBALS['comeurl']);
	}else{
		msg::msge('编辑失败');
	}
}
...
function del(){
	...
	for($ii=0;$ii<count($arrid);$ii++){
		...
		if(Core_Fun::isnumber($id)){
			$db->query("DELETE FROM ".DB_PREFIX."mapcate WHERE cid=$id");
		}
	}
	Core_Command::runlog("","删除单页分类成功[id=$arrid]");
	msg::msge('删除成功', 'xycms_mapcate.php');
}
...
$tpl->display(ADMIN_TEMPLATE."mapcate.tpl");
