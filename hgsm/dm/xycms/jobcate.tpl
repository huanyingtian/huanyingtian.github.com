...
<!--{if $action eq ""}-->
<div class="main-wrap">
  <div class="path"><p>当前位置：内容中心<span>&gt;&gt;</span>招聘分类</p></div>
  <div class="main-cont">
    <h3 class="title"><a href="xycms_jobcate.php?action=add" class="btn-general">添加分类</a>招聘分类</h3>
	<form action="xycms_jobcate.php" method="post" name="myform" id="myform" style="margin:0">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table" align="center">
	  ...
	  <tr>
		...
		<th width="10%"><div class="th-gap">招聘信息</div></th>
	  ...
	  <tr onmouseover="overColor(this)" onmouseout="outColor(this)">
		...
		<td align="center"><!--{$volist.jobcount}--></td>
		<td align="center"><!--{$volist.timeline|date_format:"%Y/%m/%d"}--></td>
		<td align="center"><a href="xycms_jobcate.php?action=edit&id=<!--{$volist.cid}-->&page=<!--
{$page}-->" class="icon-set">设置</a>&nbsp;&nbsp;<a href="xycms_jobcate.php?action=del&id[]=<!--{$volist.cid}-->" onClick="{if(confirm('确定要删除该信息?')){return true;} return false;}" class="icon-del">删除</a></td>
	  </tr>
	  <!--{foreachelse}-->
      <tr>
...
<!--{if $action eq "add"}-->
<div class="main-wrap">
  <div class="path"><p>当前位置：内容中心<span>&gt;</span>添加招聘分类</p></div>
  <div class="main-cont">
	<h3 class="title title_padding"><a href="xycms_jobcate.php" class="btn-general">返回列表</a><span class="pro_hover">招聘分类基本信息</span><span>招聘分类配置信息</span></h3>
    <form name="myform" id="myform" method="post" action="xycms_jobcate.php" onsubmit='return checkform();' >
    <input type="hidden" name="action" value="saveadd" />
	<table class='table_form'>
	  <tr>
		<td width="70">分类名称：<span class='f_red'>*</span></td>
...
<!--{if $action eq "edit"}-->
<div class="main-wrap">
  <div class="path"><p>当前位置：内容中心<span>&gt;</span>编辑招聘分类</p></div>
  <div class="main-cont">
	<h3 class="title title_padding"><a href="xycms_jobcate.php?<!--{$comeurl}-->" class="btn-general">返回列表</a><span class="pro_hover">招聘分类基本信息</span><span>招聘分类配置信息</span></h3>
    <form name="myform" id="myform" method="post" action="xycms_jobcate.php" onsubmit='return checkform();' >
	...
	<table class='table_form'>
	  ...
	  <tr>
		<td>打开方式：<span class='f_red'></span></td>
		<td><select name="target" id="target"><option value="1"<!--{if $cate.target==1}--> selected<!--{/if}-->>本页面</option><option value="2"<!--{if $cate.target==2}--> selected<!--{/if}-->>新页面</option></select></td>
	  </tr>
	  <tr>
		<td>链接类型：<span class='f_red'></span></td>
		<td><input type="radio" name="linktype" value="1"<!--{if $cate.linktype==1}--> checked<!--{/if}--> />内部链接，<input type="radio" name="linktype" value="2"<!--{if $cate.linktype==2}--> checked<!--{/if}--> />外部链接</td>
	  </tr>
	  <tr>
		<td>外部URL：<span class='f_red'></span></td>
