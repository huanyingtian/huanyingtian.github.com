...
function volist(){
    ...
    $pagesize	= 30;
	$searchsql	= " WHERE 1=1";
	$countsql	= "SELECT COUNT(cid) FROM ".DB_PREFIX."jobcate".$searchsql;
	...
	foreach($cate as $key=>$value){
		$cate[$key]['jobcount'] = $db->fetch_count("SELECT COUNT(id) FROM ".DB_PREFIX."job WHERE cid=".$value['cid']."");
	}
...
function edit(){
	...
	$sql	= "SELECT * FROM ".DB_PREFIX."jobcate WHERE cid=$id";
...
function saveadd(){
	...
	if($result){
		Core_Command::runlog("","添加招聘分类成功[$cname]",1);
		msg::msge('保存成功','xycms_jobcate.php');
	}else{
		msg::msge('保存失败');;
	}
}
...
function saveedit(){
	...
	$result = $db->update(DB_PREFIX."jobcate",$array,"cid=$id");
	if($result){
		Core_Command::runlog("","编辑招聘分类成功[id=$id]");
		msg::msge('编辑成功','xycms_jobcate.php?'.$GLOBALS['comeurl']);
	}else{
		msg::msge('编辑失败');
	}
}
...
function del(){
	...
	for($ii=0;$ii<count($arrid);$ii++){
		...
		if(Core_Fun::isnumber($id)){
			$sql	= "SELECT img FROM ".DB_PREFIX."jobcate WHERE cid=$id";
			$rows	= $db->fetch_first($sql);
			if($rows){
				$db->query("DELETE FROM ".DB_PREFIX."jobcate WHERE cid=$id");
			}else{
				msg::msge('找不到你要删除的数据');
			}
		}
	...
	msg::msge('删除成功','xycms_jobcate.php');
}
...
function updateajax($_id,$_action){
    ...
    if(Core_Fun::isnumber($_id)){
		...
		switch($_action){
			case 'flagopen':
				$db->query("UPDATE ".DB_PREFIX."jobcate SET flag=1 WHERE cid=$_id");
				break;
			case 'flagclose':
				$db->query("UPDATE ".DB_PREFIX."jobcate SET flag=0 WHERE cid=$_id");
				break;
			case 'eliteopen':
				$db->query("UPDATE ".DB_PREFIX."jobcate SET elite=1 WHERE cid=$_id");
				break;
			case 'eliteclose':
				$db->query("UPDATE ".DB_PREFIX."jobcate SET elite=0 WHERE cid=$_id");
				break;
			default:
				break;
		}
