...
foreach($product as $key=>$data){
  $product[$key]['url']    = PATH_URL.'m/product/'.$data['id'].'.html';	
  $product[$key]['caturl'] = PATH_URL.'m/product/'.$data['word'].'/';
  $product[$key]['thumbfiles'] = PATH_URL.$data['thumbfiles'];
}
foreach($info as $key=>$data){
	$info[$key]['url']    = PATH_URL.'m/news/'.$data['id'].'.html';
	$info[$key]['caturl'] = PATH_URL.'m/news/'.$data['word'].'/';
	$text = $data['content'];
...
$tpl->assign('metakeyword',$page_keyword);
