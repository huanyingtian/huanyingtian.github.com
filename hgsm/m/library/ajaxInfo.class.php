...
class ajaxInfo{
	...
	public function get_page($parm,&$parm2)
	{
	     ...
	     {
			 $result[$i]['word']=PATH_URL.'m/about_mobile/'.$result[$i]['word'].'.html';
		 }
		 return $result;
	}
		...
		foreach ($result as $key => $value) {
			if($value['linktype'] == 2 && $value['linkurl'] != ''){
				...
				if(!empty($result_chil)){
					$result[$key]['url'] = PATH_URL.'m/about_'.$value['word'].'/';
				}else{
					$result[$key]['url'] = PATH_URL.'m/about_'.$title.'/'.$result[$key]['word'].'.html';
				}
			}
		}
		// print_r($result);exit;
	...
	public function pagecate(){
		...
		foreach ($cate as $key => &$val) {
			$val['url'] = PATH_URL.'m/about_'.$val['catdir'].'/';
		}
		return $cate;
	}
	...
	public function news_list($parm1,&$var)
	{
	    ...
	    {
			$news[$i]['url']=PATH_URL.'m/news/'.$news[$i]['id'].'.html';
		}
		return $news;
	}
	...
	public function news_sort($word = '')
	{
		...
		if(empty($word)){
			...
			for($i=0;$i<count($cate);$i++)
			{
				...
				$result = $db->getall($sql);
				if(empty($result)){
					$cate[$i]['word']=PATH_URL.'m/news/'.$cate[$i]['word'].'/';
				}else{
					$cate[$i]['word']=PATH_URL.'m/news_'.$cate[$i]['word'].'/';
				}
			...
			foreach ($cate as $key => &$val) {
				...
				$result_chil = $db->getall($sql_chil);
				if(empty($result_chil)){
					$val['word'] = PATH_URL.'m/news/'.$val['word'].'/';
				}else{
					$val['word'] = PATH_URL.'m/news_'.$val['word'].'/';
				}
			}
		}
		return $cate;
...
public function get_index_case()
	{
		global $db;
	    $sql = 'SELECT `id`,`thumbfiles`,`title`,`content`,`timeline`,`flag` FROM '.DB_PREFIX."case WHERE `flag` = 1 AND elite = 1 " .
		"ORDER BY orders DESC {$limit}";
		$case = $db->getall($sql);
		if(empty($case)){
			$sqlnew = 'SELECT `id`,`thumbfiles`,`title`,`content`,`timeline`,`flag` FROM '.DB_PREFIX."case WHERE `flag` = 1 " .
			"ORDER BY orders DESC {$limit}";
			$case = $db->getall($sqlnew);
		}
		for($i=0;$i<count($case);$i++)
		{
		   $case[$i]['thumbfiles']=PATH_URL.$case[$i]['thumbfiles'];
		   $case[$i]['url']='./case/'.$case[$i]['id'].'.html';
		}
		if(count($case) <= 6)
		{
			return $case;
		}
		return array_slice($case,0,6,true);
	}
	...
	public function news_index(){
		...
		return array_slice($news,0,3,true);
	}
		...
		if(empty($word)){
			...
			for($i=0;$i<count($cate);$i++)
			{
				if($cate[$i]['linktype'] == 2 && $cate[$i]['linkurl'] != ''){
					...
					$result = $db->getall($sql);
					if(empty($result)){
						$cate[$i]['word'] = PATH_URL.'m/product/'.$cate[$i]['word'].'/';
					}else{
						$cate[$i]['word'] = PATH_URL.'m/product_'.$cate[$i]['word'].'/';
					}
				}
			}
		}else{
			...
			foreach ($cate as $key => &$val) {
				if($val['linktype'] == 2 && $val['linkurl'] != ''){
					...
					$result_chil = $db->getall($sql_chil);
					if(empty($result_chil)){
						$val['word'] = PATH_URL.'m/product/'.$val['word'].'/';
					}else{
						$val['word'] = PATH_URL.'m/product_'.$val['word'].'/';
					}
				}
			}
		}
		...
		if(empty($word)){
			...
			for($i=0;$i<count($cate);$i++)
			{
				if($cate[$i]['linktype'] == 2 && $cate[$i]['linkurl'] != ''){
					...
					$result = $db->getall($sql);
					if(empty($result)){
						$cate[$i]['word'] = PATH_URL.'m/case/'.$cate[$i]['word'].'/';
					}else{
						$cate[$i]['word'] = PATH_URL.'m/case_'.$cate[$i]['word'].'/';
					}
				}
			}
		}else{
			...
			foreach ($cate as $key => &$val) {
				if($val['linktype'] == 2 && $val['linkurl'] != ''){
					...
					$result_chil = $db->getall($sql_chil);
					if(empty($result_chil)){
						$val['word'] = PATH_URL.'m/case/'.$val['word'].'/';
					}else{
						$val['word'] = PATH_URL.'m/case_'.$val['word'].'/';
					}
				}
			}
		}
	...
	public function get_sort_list($aa)
	{
		...
		for($i=0;$i<count($products);$i++)
		{
			...
			$products[$i]['url']=PATH_URL.'m/product/'.$products[$i]['id'].'.html';
		}
	...
	public function case_sort_list($aa)
	{
		...
		for($i=0;$i<count($cases);$i++)
		{
			...
			$cases[$i]['url']=PATH_URL.'m/case/'.$cases[$i]['id'].'.html';
		}
		...
		for($i=0;$i<count($cate);$i++){
		   if($cate[$i]['linktype'] == 2 && $cate[$i]['linkurl'] != ''){
				$cate[$i]['url'] = $cate[$i]['linkurl'];
					...
					$cate[$i]['url'] = PATH_URL.'m/job/'.$cate[$i]['cid'].'/';
			}
		}
		return $cate;
	}
	...
	{
		global $db;
		$sql = 'SELECT `id`,`thumbfiles`,`title`,`number`,`jobdescription`,`jobrequest`,`jobotherrequest`,`jobcontact`,`timeline`,`flag` FROM '.DB_PREFIX."job AS v WHERE `flag` = 1 and `cid`='{$aa}'  ORDER BY id DESC {$limit}";
		$jobs = $db->getall($sql);
		for($i=0;$i<count($jobs);$i++)
		{
			$jobs[$i]['thumbfiles']=PATH_URL.$jobs[$i]['thumbfiles'];
			$jobs[$i]['url']=PATH_URL.'m/job/'.$jobs[$i]['id'].'.html';
		}
