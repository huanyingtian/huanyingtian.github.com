	...
	public function content_change(&$content){
		...
		if($this->table == "product"){
			foreach($content as $key=>$value){
				$content[$key]['url']      = M_URL."{$this->table}/{$value['id']}.html";
				$content[$key]['caturl']   = M_URL."product/{$value['word']}/";
				$content[$key]['summary']  = Core_Fun::de_cut($value['content'], 28,'...');
				if(isset($value['target']) && $value['target'] == 2){
					$content[$key]['target'] = "_blank";
				}
				if($value['thumbfiles'] == ''){
					$content[$key]['thumbfiles'] = M_URL."template/images/nopic.jpg";
				}else{
