...
class chency_mysql {
	...
	function query($sql, $type = '') {
		...
		if(!($query = $func($sql, $this->link))) {
			if(in_array($this->errno(), array(2006, 2013)) && substr($type, 0, 5) != 'RETRY') {
				$this->close();
				require CHENCY_ROOT.'./libs/db.inc.php';
	...
	function halt($message = '', $sql = '') {
		$dberror = $this->error();
		$dberrno = $this->errno();
		echo "<meta http-equiv='Content-Type' content='text/html;charset=".PHPOE_CHARSET."' /><div style=\"position:absolute;font-size:11px;font-family:verdana,arial;background:#EBEBEB;padding:0.5em;\"><b>MySQL Error</b><br><b>Message</b>: $message<br><b>SQL</b>: $sql<br><b>Error</b>: $dberror<br><b>Errno.</b>: $dberrno<br></div>";
		exit();
	}
    ...
    function insert($table,$array,$is=0) {
		$fields = $values = array();
		$table=str_replace('%s',DB_PREFIX,$table);
